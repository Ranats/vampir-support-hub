...
進捗、レベル、自分の項目、出現通知対象、クラン予定とそのタイムゾーン、通知設定は端末内に保存されます。

主な機能：

- 次のイベントボス／ワールドボス／ゲヘナとカウントダウン（イベントボスは確認済みの地域別時刻に対応）
- 毎日05:00、毎週月曜05:00のチェックリセット
- レベルによる表示絞り込みと自分用項目
- 通知対象にした出現予定の5／10／30分前通知（サイト表示中のみ）
- クラン任務・守護のIANAタイムゾーン別曜日／時刻、週次参加チェック、事前リマインド
...
- 閲覧リンクと管理リンクを分けたクラン共有ポータル

通知は利用者が設定画面から明示的に許可した場合だけ使用します。サーバー側の定刻
Pushは使っていないため、ページを閉じた後の通知は保証しません。ゲーム内表示と
公式告知を常に正本とします。

公式の出現・イベント時刻は原則JSTです。公式に地域別時刻を確認できたイベントボスだけ、保存したサーバー地域に合わせて表示します。日次05:00／週次月曜05:00のリセット周期は常にJSTです。
ユーザー入力のクラン予定だけは別にIANAタイムゾーンを選択でき、夏時間の切り替えを
週次の現地時刻として計算します。既存のクラン予定はJSTのまま移行されます。

支援リンクは任意です。決済処理や支援先の追跡スクリプトはサイト内へ埋め込まず、
すべての機能を支援の有無にかかわらず無料で利用できます。

日本語・英語の公開トップページ、出現時刻ページ、運営・プライバシー方針ページではCloudflare Web Analyticsを使用し、
...
クラン共有ポータルを明示的に作成した場合だけ、クラン名、開催曜日・時刻、クラン予定のタイムゾーンをD1へ保存します。
個人のチェック状況、レベル、通知設定は引き続き端末内に保持し、共有予定とは分離します。
個人バックアップv2にはクラン予定のタイムゾーンを含め、旧v1バックアップはJSTとして読み込みます。
