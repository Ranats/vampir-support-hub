...
export default function ClanPortalCreateClient({ locale = "ja" }: { locale?: Locale }) {
  const en = locale === "en";
  ...
  async function createPortal(event: FormEvent<HTMLFormElement>) {
    ...
    if (!displayName.trim()) {
      setError(en ? "Enter a clan name." : "クラン名を入力してください。");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(en
          ? "The shared portal could not be created."
          : typeof payload.error === "string" ? payload.error : "作成できませんでした。");
      }
      ...
      if (!portal || !isClanPortalToken(viewToken) || !isClanPortalToken(adminToken)) {
        throw new Error(en ? "The creation result could not be verified." : "作成結果を確認できませんでした。");
      }

      const origin = window.location.origin;
      const createdLinks = {
        viewUrl: buildClanPortalUrl(origin, portal.id, "viewer", viewToken, locale),
        adminUrl: buildClanPortalUrl(origin, portal.id, "admin", adminToken, locale),
      };
      ...
      try {
        ...
        setMessage(en
          ? "The shared clan portal was created. Store the admin link securely because it cannot be shown again."
          : "クラン共有ポータルを作成しました。管理リンクは再表示できないため、安全な場所に保管してください。");
      } catch {
        setMessage(en
          ? "The portal was created, but the admin key could not be saved on this device. Copy the admin link before leaving this page."
          : "ポータルは作成できましたが、この端末へ管理キーを保存できません。移動する前に管理リンクを必ずコピーしてください。");
      }
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : en ? "The shared portal could not be created." : "作成できませんでした。");
    } finally {
      setCreating(false);
    }
  }
    ...
    <main className="clan-portal-shell">
      ...
      <div className="clan-portal-main">
        <div className="clan-portal-heading">
          ...
          <h1>{en ? "Create a shared clan portal" : "クラン共有ポータルを作成"}</h1>
          <p>{en
            ? "When a clan master saves weekdays and times, every member with the viewer link can see the same schedule."
            : "クランマスターが曜日と時刻を保存すると、閲覧リンクを持つメンバー全員が同じ予定を確認できます。"}</p>
        </div>

        {links ? (
          <section className="clan-portal-created panel" aria-labelledby="portal-created-title">
            <div>
              <span className="status-pill good">{en ? "Created" : "作成完了"}</span>
              <h2 id="portal-created-title">{en ? "Save both links" : "2種類のリンクを保存してください"}</h2>
              {message ? <p className="form-success" role="status">{message}</p> : null}
            </div>
            <div className="portal-link-card">
              <div><strong>{en ? "Member viewer link" : "メンバー閲覧リンク"}</strong><small>{en ? "Share this link with clan members" : "クランメンバーへ共有するリンク"}</small></div>
              <input readOnly value={links.viewUrl} aria-label={en ? "Member viewer link" : "メンバー閲覧リンク"} />
              <button type="button" onClick={() => void copyText(links.viewUrl)}>{en ? "Copy" : "コピー"}</button>
            </div>
            <div className="portal-link-card danger">
              <div><strong>{en ? "Clan master admin link" : "クランマスター管理リンク"}</strong><small>{en ? "Can edit schedules, replace the viewer link, and delete the portal. Do not share it." : "予定の変更・閲覧リンク再発行・削除が可能。共有しないでください。"}</small></div>
              <input readOnly value={links.adminUrl} aria-label={en ? "Clan master admin link" : "クランマスター管理リンク"} />
              <button type="button" onClick={() => void copyText(links.adminUrl)}>{en ? "Copy" : "コピー"}</button>
            </div>
            <a className="primary-action portal-open-link" href={links.adminUrl}>{en ? "Open admin page" : "管理ページを開く"}</a>
          </section>
        ) : (
          <form className="clan-portal-form panel" onSubmit={createPortal}>
            <label className="portal-name-field">
              ...
              <input
                ...
                placeholder={en ? "Example: Crimson Night" : "例：深紅の夜"}
            ...
            <button className="primary-action portal-submit" type="submit" disabled={creating}>
              {creating
                ? (en ? "Creating…" : "作成中…")
                : (en ? "Create shared portal" : "共有ポータルを作成")}
            </button>
          </form>
        )}
      </div>
