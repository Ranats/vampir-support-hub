...
- Preserve: the public Cloudflare Web Analytics site token `11a11bdb70184f96822eb5d171c6687b` is an embed identifier, not an API credential. Render its manual beacon exactly once in the initial HTML for `/` and `/policy`, with `spa: false`, and never render it from the root layout or under `/clan/*`.
...
- Acceptance criteria: the initial HTML for `/` and `/policy` each contains exactly one manual `https://static.cloudflareinsights.com/beacon.min.js` script with the configured public site token and `spa: false`; `/clan/*` contains none, including after SPA navigation or an offline navigation fallback. The service worker must not intercept or cache `/clan` or `/clan/*`.
