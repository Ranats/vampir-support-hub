...
- Preserve: `vampir-event-progress-v1` for manually entered event and mission progress. Campaign and objective IDs stay versioned, daily and weekly objectives follow the existing JST reset cycles, and progress stays device-local without implying game-account synchronization.
...
- Acceptance criteria: backup v3 round-trips the separate clan time zone and event progress only after whole-file validation; backup v1 migrates to `Asia/Tokyo`, backup v1/v2 migrate to empty event progress, and invalid time-zone or event-progress data rejects the whole file; expired imported cycle progress is not restored into the current cycle; same-browser tabs converge without changing established storage shapes.
- Acceptance criteria: Japanese and English manifests use their matching language and start route, and both include 192px and 512px raster icons; bundled webfonts are emitted as public build assets, and rendered output must not expose build-machine or `.vinext/fonts` paths; service-worker requests remain network-first for time-sensitive content; notification permission is never requested on first load; no copy implies notification after the site is closed.
...
- Acceptance criteria: each active event card shows completed objectives, remaining amounts, the next action, and dated official details; count progress is clamped to its verified target; daily and weekly event objectives reset with the existing JST cycles; event progress survives backups and same-browser tab sync.
